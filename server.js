...
const JWT_SECRET = process.env.JWT_SECRET || "milkgram-dev-secret-change-me"; // ВАЖНО: смени в проде через переменную окружения
...
// Список пользователей (для MVP — просто все, кроме себя; на 100k нужен будет поиск с пагинацией)
...
io.on("connection", (socket) => {
  ...
  socket.on("private_message", ({ to, text }) => {
    ...
    // отправляем себе (подтверждение) и адресату, если он онлайн
  ...
  socket.on("disconnect", () => {
    ...
    // если во время звонка собеседник отключился — сообщаем второй стороне
    socket.broadcast.emit("call:end", { from: socket.userId });
  });

  // ---------- Звонки (WebRTC signaling) ----------
  // Сервер тут не передаёт голос — он только "сводит" двух людей,
  // а сам звук/видео идёт напрямую между их устройствами.
  ...
  socket.on("call:offer", ({ to, offer }) => {
    const targetSocketId = onlineUsers.get(to);
    if (targetSocketId) io.to(targetSocketId).emit("call:offer", { from: socket.userId, offer });
  });

  socket.on("call:answer", ({ to, answer }) => {
    const targetSocketId = onlineUsers.get(to);
    if (targetSocketId) io.to(targetSocketId).emit("call:answer", { from: socket.userId, answer });
  });

  socket.on("call:ice-candidate", ({ to, candidate }) => {
    const targetSocketId = onlineUsers.get(to);
    if (targetSocketId) io.to(targetSocketId).emit("call:ice-candidate", { from: socket.userId, candidate });
  });

  socket.on("call:reject", ({ to }) => {
    const targetSocketId = onlineUsers.get(to);
    if (targetSocketId) io.to(targetSocketId).emit("call:reject", { from: socket.userId });
  });

  socket.on("call:end", ({ to }) => {
    const targetSocketId = onlineUsers.get(to);
    if (targetSocketId) io.to(targetSocketId).emit("call:end", { from: socket.userId });
  });
});
